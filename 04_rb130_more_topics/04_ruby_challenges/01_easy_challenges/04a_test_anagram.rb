...
class AnagramTest < Minitest::Test
  ...
  # My tests
  def test_duplicate_anagrams
    detector = Anagram.new('orchestra')
    anagrams = detector.match %w(cashregister carthorse radishes carthorse)
    assert_equal ['carthorse', 'carthorse'], anagrams
  end
  
  def test_same_order_of_input_list
    detector = Anagram.new('master')
    anagrams = detector.match(%w(stream pigeon maters))
    assert_equal %w(stream maters), anagrams
  end
  
  def test_empty_seed_word
    detector = Anagram.new('')
    anagrams = detector.match(%w(stream pigeon maters))
    assert_equal [], anagrams
  end
  
  def test_empty_seed_word_with_empty_string
    detector = Anagram.new('')
