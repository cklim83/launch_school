...
Examples
- Array can have any number of elements, block accept 1 parameter but may use or not use it
- Return true even if array is empty
- Return false the moment block returns false for any element
- Return true only if all return value is truthy

Algorithm
- Iterate through each element and yield to block
- return false unless block returns true
- return true
=end
...
Our solution is extremely similar to our solution for any?. It simply iterates
through our collection, and returns false if it encounters an item that produces
a false result when it is yielded to the block. If no such item is encountered,
we return true.
=end