...
=begin
My answer:
Array#sort relies on <=> of its elements for sorting its elements.
Since Length does not have <=> implemented, it results in this error.
We implement <=> by making use of the other comparison already implemented
=end
...
done by the <=> method. Therefore, the objects in the collection we are
sorting must have access to a <=> method. Since our Length class does not
implement a <=> method, we get an error saying that the "comparison of Length
with Length failed".
...
Comparable (see this link), as it implements all of the comparison methods
based on <=>. However, you don't need to perform this step.
...
CONCEPTS
- Array#sort relies on array elements implementing #<=> for sorting
- For any class, we can implement <=>, then include Comparable and it will auto generate the 
  other comparison method e.g. >, <, == etc (see below for example)
=end
...
# p Length.new(3, :km) >= Length.new(2, :km) # true